...
ISR(INT0_vect) {
	static unsigned rxBits = 0;
	static unsigned scanBit = 1;
	
	if(PINB & (1 << PS2_DATA)) rxBits = (scanBit | rxBits);
	...
	if(scanBit == 0x800) {
		s = (rxBits>>1) & 0xff;
		rxBits = 0, scanBit = 1;
		if (s==0xaa) return; // Ignore BAT completion code
		if (s==0xf0) { sBreak=1; return; }
		if (s==0xe0) { sModifier=1; return;}
		if (sBreak) {// if key released
			if ((s==0x12) || (s==0x59)) {sShift=0;} // Left or Right Shift Key
			sBreak=0; sModifier=0; return;
		}
		if ((s==0x12) || (s==0x59)) sShift=1;// Left or Right Shift Key
		if (sModifier) return;  // If modifier ON, do nothing
		if (sShift==0)
		c=pgm_read_byte(keymap_unshifted+s);
		else
		c=pgm_read_byte(keymap_shifted+s);
		if (c == 32 && s != 0x29) return; // Ignore unless real space key
